from flask import Flask, request, render_template_string, render_template, session, redirect, url_for, g
...
app.config["SESSION_COOKIE_SECURE"] = bool(os.environ.get("FORCE_HTTPS", False))  # True si usas HTTPS
...
def login():
    if request.method == 'POST':
        ...
        # Validaciones básicas (evitar strings vacíos)
        if not username or not password:
            return render_template_string(login_template, error="Username and password are required.")

        conn = get_db_connection()
        try:
            # Buscar por username solo, luego se verifica la contraseña (password) con check_password_hash
...
# Template separado para login (token CSRF)
login_template = '''
<!doctype html>
<html lang="en">
<head>
...
def dashboard():
    ...
    return render_template_string(dashboard_template, user_id=user_id, comments=comments)
